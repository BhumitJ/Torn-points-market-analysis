This is the Power Query to extract market data for any item from the Torn API. It is within torn_excel_query.xlsm. Clone or download the repo to get started.

## Why not pull directly
The data pulled directly is in JSON and not in a tabular form. It is mostly under records and different layers. It can become messy and complicated to arrange neatly for effective viewing.
...
3. The KEY is pulled in from the Home sheet, so there's no need to do anything here. Put your key in the designated space in the Home sheet. 
...
7. After getting the info, you can click on Close and Load. If you have results for more than one query, you won't be able to load them in the current worksheet. One way is to load them as connection-only, then load them in the worksheet from the current queries list on the right-hand side. Right-click the query, then click Load for options. On closing the Power Query Editor directly with the close button, it will ask in a dialogue where to save it. If you save it will load all queries in different sheets.
8. Load as tables in the desired place. With each refresh, the values will be updated. You can refresh from the button in the Home sheet, or from the Data ribbon, or from Power Query Editor.
9. The museum item sets in excel/torn_points_market_dashboard.xlsm have all been loaded in this way. 

## Obtaining item ID for different items
There are a couple of ways
1. You can directly use your key in this URL: https://api.torn.com/torn/?selections=items&key=YOUR_KEY_HERE
   This will give you a list of all items in Torn with their item ID. Search for your item using Ctrl+F and find out the item ID.
...
There is a limit for requests per API in Torn (100 requests per minute). If you refresh too many times and encounter such an error, wait for a minute and try again.
